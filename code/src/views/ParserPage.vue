    ...
    brancaSplittedIncomesList({ incomes }) {
      ...
      incomes.forEach( income => {
        ...
        for (const branca of branche) {
          ...
          let indexOfFineCausale = description.search('Info aggiuntive:')
          if (indexOfBranca != -1) {
            income.note = description.slice(indexOfBranca, indexOfFineCausale).replace(branca.regex, '')
      ...
      let parsedData = allRows
        .map(row => {
          ...
          let valueStr = (rowCells[2] || '')
            ...
            .replace(',', '.')
          return {
            dataContabile: rowCells[0],
            dataValuta: rowCells[1],
